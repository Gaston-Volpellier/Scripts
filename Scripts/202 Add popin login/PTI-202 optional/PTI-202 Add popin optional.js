...
const fullscreenPopin = "${isFullscreen}";
...
const createMiniPopinElt = () => {
  const newCtn = createElt("div", {
    class:
      "dy-popin-small curved--large bg-white l-fixed font-small card-shadow--light dy-arrow ${Popin Desktop Size} ${Popin Mobile Tablet} ${Popin Mobile Size}",
  });
  ...
  const userIcon = createElt(
    "div",
    { class: "l-hmargin--small dy-iconSize" },
  ...
  const firstContent = createElt("div", { class: "fw-bold" }, "${Title small}");
...
const init = () => {
  ...
  const popinElt =
    fullscreenPopin === "true" ? createPopinElt() : createMiniPopinElt();
  ...
  if (fullscreenPopin === "true") {
    ...
    if (btnLogin) {
      const headerMenuMain = document.querySelector(".header-menu-main");
      const headerMenuMainParent = headerMenuMain.parentNode;
  ...
  if (fullscreenPopin === "true" && popupCtn) {
