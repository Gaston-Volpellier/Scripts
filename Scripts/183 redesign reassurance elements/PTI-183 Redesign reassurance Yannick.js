...
function createNewCtnReinsurance(refNode, parentNode) {
  ...
  newCtnReinsurance.setAttribute(
    "class",
    "dy-reinsurance l-display-grid l-column-gap--medium l-row-gap--medium grid-template-2 grid-template-m-1 text-black fs--medium ff-normal l-m-padding--small"
  );
...
function createCell(data, picKlana) {
  ...
  div.setAttribute(
    "class",
    "dy-reinsurance-items curved--medium bordered-grey-medium l-padding--small l-padding-around flex flex--col flex-m--row flex--space-between flex--align-center flex-m--align-start"
  );
  if (data !== null) {
    if (data.link3 !== "none") {
    ...
    innerHTML +=
      '<div class="flex flex--col flex--align-center flex-m--align-start flex-m--basis-full">';
    innerHTML +=
